...
async function unlist_repo(params, model) {
...
async function repo_detail(params, model) {
  return await model.for_sell.findOne({
    where: { username: params.username, name: params.name },
  });
}
...
async function sellRepo({ model, token_pass, token, name, amount }) {
  ...
  if (_user) {
    ...
    if (repo) {
      ...
      const updated = await _repo.update({
        ...repoDetail,
        sell: "SELL",
        amount: amount,
      });
      if (updated || created) {
        const forSale = await model.for_sell.findAll({
...
async function listRepo({ model, token_pass, token, item }) {
  ...
  if (_user) {
    const { id, ...other } = item;
    const [repo, created] = await model.for_sell.findOrCreate({
      where: { repo_id: id },
      defaults: {
        ...other,
        sell: "UNLIST",
    ...
    if (repo) {
      ...
      if (created || updated) {
        const repoAll = await model.for_sell.findAll({
          where: { username: _user.username },
        });
        return repoAll;
      } else {
